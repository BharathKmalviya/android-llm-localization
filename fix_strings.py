...
def main():
    ...
    for xml_file in files:
        ...
        def fix_match(m):
            ...
            # Fix % symbols:
            # 1. First drop all consecutive '%' to a single '%'
            # This fixes any '%%%' or '%%%%' that might have been introduced
            text = re.sub(r'%+', '%', text)
            
            # 2. Re-escape purely literal % symbols to '%%'
            # The only valid formats in our app are %1$d, %1$s, %d, and %s.
            text = re.sub(r'%(?!1\$d|1\$s|d|s)', '%%', text)
            
            return prefix + text + suffix

        new_content = re.sub(r'(<string[^>]*name="[^"]*"[^>]*>)(.*?)(</string>)', fix_match, content, flags=re.DOTALL)
